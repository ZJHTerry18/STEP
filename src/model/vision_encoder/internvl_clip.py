...
class InternVLTokenizer(nn.Module):
    def __init__(self):
        ...
        self.tokenizer = LlamaTokenizer.from_pretrained(
            "/mnt/petrelfs/share_data/chenzhe1/data/llm/chinese_alpaca_lora_7b", 
    ...
    def forward(self, text):
        ...
        text = self.tokenizer(text, return_tensors="pt", padding=True).input_ids
        return text
...
def get_model_and_transform(img_size, device):
    llama_path = "/mnt/petrelfs/share_data/chenzhe1/data/llm/chinese_alpaca_lora_7b"
    model = InternVL_CLIP(img_size=img_size,
...
def load_internvl_clip(model_name, pretrained, cache_dir, device):
    tokenizer = InternVLTokenizer()
    model, transform = get_model_and_transform(img_size=224, device=device)
...
def load_internvl_clip_flickr_finetuned(model_name, pretrained, cache_dir, device):
    tokenizer = InternVLTokenizer()
    model, transform = get_model_and_transform(img_size=448, device=device)
    ckpt_path = f'/mnt/petrelfs/wangwenhai/workspace/InternVL/retrieval_finetune/exp/configs/' \
                f'6b_vit/6b_vit_{model_name}_clip_alpaca_7b_flickr30k_peak_1e-5_32gpu.sh/' \
                f'{pretrained}/mp_rank_00_model_states.pt'
    ckpt = torch.load(ckpt_path, map_location='cpu')
    new_ckpt = process_checkpoint(ckpt)
    message = model.load_state_dict(new_ckpt, strict=False)
...
def load_internvl_clip_196(model_name, pretrained, cache_dir, device):
    tokenizer = InternVLTokenizer()
    model, transform = get_model_and_transform(img_size=196, device=device)
    ckpt_path = f'/mnt/petrelfs/wangwenhai/workspace_swj/code/sim_clip_wj_deepspeed/exp/configs/' \
                f'6b_vit/6b_vit_{model_name}_clip_alpaca_7b_laion5b_peak_1e-5_256gpu_all_trainable_degradation.sh/' \
                f'{pretrained}/mp_rank_00_model_states.pt'
    ckpt = torch.load(ckpt_path, map_location='cpu')
...
def load_internvl_clip_336(model_name, pretrained, cache_dir, device):
    tokenizer = InternVLTokenizer()
    model, transform = get_model_and_transform(img_size=336, device=device)
    ckpt_path = f'/mnt/petrelfs/wangwenhai/workspace_swj/code/sim_clip_wj_deepspeed/exp/configs/6b_vit/' \
                f'6b_vit_{model_name}_clip_alpaca_7b_laion5b_peak_1e-5_256gpu_all_trainable_degradation.sh/' \
                f'{pretrained}/mp_rank_00_model_states.pt'
    ckpt = torch.load(ckpt_path, map_location='cpu')
    new_ckpt = process_checkpoint(ckpt)
    message = model.load_state_dict(new_ckpt, strict=False)
...
if __name__ == '__main__':
    tokenizer = LlamaTokenizer.from_pretrained("/mnt/petrelfs/share_data/chenzhe1/data/llm/chinese_alpaca_lora_7b")
