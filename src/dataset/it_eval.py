...
class ITEvalDataset(BaseDataset):
    ...
    def process_qa(self, qa, visual, msg=""):
        # cur_instruction = ""
        ...
        cur_instruction = 'Carefully watch the video and pay attention to the cause and sequence of events, the detail and movement of objects, and the action and pose of persons. Based on your observations, select the best option that accurately addresses the question.\n'
        
        input_ids, attention_masks, index = [], [], []
        ...
        if self.media_type == 'image':
            index.append(tokenized['image_index'])
        else:
            index.append(tokenized['video_index'])
        ...
        for sentence in qa:
            ...
            q = sentence["q"]
            if q != "":
                conversation = (" " + "[INST]" + " " + q + "\nOnly give the best option. " + "[/INST]")
            else:
            ...
            input_ids.append(tokenized['input_ids'])
            attention_masks.append(tokenized['attention_mask'])
            index.append(torch.zeros_like(tokenized['input_ids']))
            break
        
        input_ids = torch.cat(input_ids)[:self.max_length]
        attention_masks = torch.cat(attention_masks)[:self.max_length]
        index = torch.cat(index)[:self.max_length]
        return input_ids, attention_masks, index
    ...
    def __getitem__(self, index):
        try:
            ...
            input_ids, attention_masks, idx = self.process_qa(ann["qa"], visual=image)
