...
from src.share_utils.distributed import is_dist_avail_and_initialized
        ...
        while True:
            try:
                for name in self.iter_order:
                    # print('Using datatype:', name)
                    _iter = self.name2iter[name]
                    batch = next(_iter)
                    yield batch
            except Exception as e:
                logger.warning(f"Caught exception {e}.")
                self.name2iter = {name: iter(l) for name, l in self.name2loader.items()}
