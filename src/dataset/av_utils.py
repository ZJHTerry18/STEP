...
def load_full_audio_av(video_path, sr, max_audio_length, client):
    ...
    if csr != sr:
        trans = torchaudio.transforms.Resample(csr, sr)
        audio = trans(audio)
    if audio.shape[1] >= max_length: # TODO 如果是test模式这里也许要改
        max_start = audio.shape[1] - max_length
        start = random.randint(0, max_start)
        audio = audio[:, start: start + max_length]
    audio = audio * 2 ** 15
...
if __name__ == '__main__':
    from petrel_client.client import Client
    client = Client('~/petreloss.conf')
    # fbank = load_full_audio_av("snew:s3://public-dataset-snew/youtobe_media/videos/44KzkZ10eGM.mp4", 16000, 10, client)
    # print(type(fbank))

    video_path = 'pssd:s3://vimeo_seg_new/256/181898487/181898487-0#00#000-0#03#600.mp4'
    # pssd:s3://vimeo_seg_new/256/181898487/181898487-0#00#000-0#03#600.wav
    video_start_frame=3063
    video_end_frame=3124
    fbank = load_full_audio_av("snew:s3://public-dataset-snew/youtobe_media/videos/44KzkZ10eGM.mp4", 16000, 10, client)
    print(type(fbank))
    # audios = load_audio_av(video_path, video_start_frame, video_end_frame, 16000, 10, client)
    # print(len(audios))
    # print(audios.dtype)
    # print(audios.shape)
    # frames = lazy_load_s3video(s3path_video=video_path,
    #                   num_frames=4,
    ...
    # # 假设img是一个PIL Image对象
    # img_tensor = pil_to_tensor(frames[0])
    ...
    # print(img_tensor[0])
    # frames = read_frames_decord('pssd:s3://ssv2_video/38262.webm',
    #                             4, client=client)