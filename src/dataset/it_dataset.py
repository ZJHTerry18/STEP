...
class ITTrainDataset(BaseDataset):
    ...
    def process_qa(self, qa, msg="", ilen=1):
        ...
        if "i" in qa[0].keys() and qa[0]["i"] != "":
            cur_instruction = "<s><|im_start|>system\n" + qa[0]["i"] + "<|im_end|>\n"
        ...
        input_ids.append(torch.ones(self.max_length).to(torch.long) * self.tokenizer.pad_token_id)
        attention_masks.append(torch.ones(self.max_length).to(torch.long))
        labels.append(torch.ones(self.max_length).to(torch.long) * -100)
        indexs.append(torch.zeros(self.max_length).to(torch.bool))
    ...
    def __getitem__(self, index):
        try:
            ...
            if self.use_dynamic_loading:
                ...
                input_ids, attention_masks, labels, indexs, inst = self.process_qa(ann["qa"], ilen=1)
            
            ret = {
                'input_ids': input_ids, 
                'attention_mask': attention_masks, 
                'labels': labels,
                'instruction': [inst] * image.shape[0],
            }
            if self.media_type == 'image':
                ret['image'] = image
