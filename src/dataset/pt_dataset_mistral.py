...
class ImgTxtPtTrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            unified_tokens = self.tokenizer.bos_token + "[INST]" + " " + '<Image>' + IMG_TOKEN + '</Image>' + " " + "[/INST]" + " "
...
class VidTxtPtTrainDataset(ImgTxtPtTrainDataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            unified_tokens = self.tokenizer.bos_token + "[INST]" + " " + '<Video>' + VID_TOKEN + '</Video>' + " " + "[/INST]" + " "
            ...
            # print('success here in line 524')
...
class ImgTxtPtFgTrainDataset(ImgTxtPtTrainDataset):
    media_type = "image"

    def __init__(self, ann_file, transform, tokenizer=None):
        super().__init__(ann_file=ann_file, transform=transform, tokenizer=tokenizer)
    
    def get_anno(self, index):
        assert self.media_type == 'image', self.media_type
        anno = {"caption": self.get_caption(index)}
        anno["image"] = self.data_root_prefix + os.path.join(self.data_root, self.anno[index]["image"])
        anno["fg_text"] = self.anno[index]["objects"] if "objects" in self.anno[index].keys() else []
        return anno
    ...
    def __getitem__(self, index):
        try:
            ...
            if len(ann['fg_text']) <= 1:
                ann['fg_text'].extend(['foreground', 'background'])
            fg_text = '|'.join(ann['fg_text'])
            return {
                'input_ids': input_ids, 
                'attention_mask': attention_mask, 
                'labels': labels,
...
class VidTxtPtFgTrainDataset(VidTxtPtTrainDataset):
    media_type = "video"
    ...
    def __init__(
        ...
        super().__init__(ann_file, transform, tokenizer, num_frames, video_reader_type, sample_type, num_tries)
