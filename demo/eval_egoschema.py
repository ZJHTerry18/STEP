...
parser.add_argument('--video_base', type=str, default='cluster2:s3://egoschema/videos/')
...
def load_video_egoschema(video_path, num_segments=8, return_msg=False, resolution=224):
...
def infer_egoschema(
    ...
    video, _ = load_video_egoschema(vid_path, num_segments=num_segments, return_msg=True)
    TC, H, W = video.shape
...
for idx, example in tqdm(enumerate(json_data)):
    ...
    flag = 1 if check_answer_egoschema(pred, idx) else 0
    correct += flag
    total += 1
