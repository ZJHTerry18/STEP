...
parser.add_argument('--video_base', type=str, default='p2:s3://ego4d/clips')
parser.add_argument('--ann', type=str, default='/mnt/petrelfs/zhaojiahe/data/ego_feedback/qaego4d_simple/test_mc.json')
...
def load_video_groundvqa(video_path, start=None, end=None, num_segments=8, return_msg=False, resolution=224):
    if "s3://" in video_path:
...
def infer_groundvqa(
    ...
    video, _ = load_video_groundvqa(vid_path, start=None, end=None, num_segments=num_segments, return_msg=True)
    TC, H, W = video.shape
...
for idx, example in tqdm(enumerate(json_data)):
    start = time.time()
    question = f"Question: {example['QA'][0]['q']}\nOptions:\n"
    for ci, choice in enumerate(example['QA'][0]['c']):
        option = chr(ord('A') + ci)
        question += f"({option}) {choice}\n"
    example['QA'][0]['q'] = question
    gt_idx = example['QA'][0]['c'].index(example['QA'][0]['a'])
    gt = f"({chr(ord('A') + gt_idx)}) {example['QA'][0]['a']}"
    
    pred = infer_groundvqa(
        example,
        model,
        tokenizer,
